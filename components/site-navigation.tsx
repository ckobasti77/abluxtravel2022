...
import { sortTripsNewestFirst, useTrips } from "../lib/use-trips";
...
export default function SiteNavigation() {
  ...
  const navItems = useMemo(() => {
    const offeredTrips = sortTripsNewestFirst(
      trips.filter((trip) => trip.status !== "completed")
    );
    const offeredJourneys = sortTripsNewestFirst(
      journeys.filter((journey) => journey.status !== "completed")
    );
